...
// RedisConfig - интерфейс определяющий методы конфига редиса
...
// RedisConfig - структура конфига редиса
...
// NewRedisConfig - метод считывающий параметры подключения из env а также возвращающий структуру
...
// Address - метод формирующий адресс подключения к редис
func (cfg *redisConfig) Address() string {
	return net.JoinHostPort(cfg.host, cfg.port)
}

// ConnectionTimeout - метод отдающий таймаут подключения
func (cfg *redisConfig) ConnectionTimeout() time.Duration {
	return cfg.connectionTimeout
}

// MaxIdle - возврат параметра maxIdle
func (cfg *redisConfig) MaxIdle() int {
	return cfg.maxIdle
}

// IdleTimeout - возврат параметра IdleTimeout
